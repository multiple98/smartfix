...
// If product doesn't exist, redirect to manage products page
if (!$product) {
    header("Location: manage_products.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (!in_array($_FILES['image']['type'], $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
                ...
                if (!empty($product['image']) && file_exists("../" . $product['image']) && $product['image'] != $new_image_path) {
    ...
    if (empty($errors)) {
        try {
            ...
            $check_updated = "SHOW COLUMNS FROM products LIKE 'updated_at'";
            $updated_stmt = $pdo->prepare($check_updated);
            $updated_stmt->execute();
            $updated_column_exists = $updated_stmt->rowCount() > 0;
            ...
            $query_parts = ["name = ?", "description = ?", "price = ?", "category = ?", "image = ?"];
            ...
            if ($updated_column_exists) {
                $query_parts[] = "updated_at = NOW()";
            }
            
            $query = "UPDATE products SET " . implode(", ", $query_parts) . " WHERE id = ?";
            $params[] = $product_id;
            ...
            if ($image_path != $product['image'] && !empty($image_path) && file_exists("../" . $image_path)) {
                unlink("../" . $image_path);
            }
        }
    }
...
<head>
    ...
    <title>Edit Product - SmartFix Admin</title>
                ...
                <form action="edit_product.php?id=<?php echo $product_id; ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select id="category" name="category" required>
                            ...
                            <option value="Computer" <?php echo ($product['category'] == 'Computer') ? 'selected' : ''; ?>>Computer</option>
                            <option value="Spare" <?php echo ($product['category'] == 'Spare') ? 'selected' : ''; ?>>Spare Parts</option>
                            <option value="Car" <?php echo ($product['category'] == 'Car') ? 'selected' : ''; ?>>Car</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="stock" name="stock" min="0" value="<?php echo isset($product['stock']) ? intval($product['stock']) : '1'; ?>" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (!empty($product['image']) && file_exists("../" . $product['image'])): ?>
                            <div class="current-image">
                                Current image: <strong><?php echo basename($product['image']); ?></strong>
                            </div>
                        ...
                        <small style="display: block; margin-top: 0.5rem; color: var(--secondary-color);">
